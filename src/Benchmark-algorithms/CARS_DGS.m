...
n_LS = max(20, floor((n_quad_pts-1)*n*0.05)); % AdaDGS paper: dim = 1000. Since n << 1000 use larger default value here
...
for k=1:maxit
    ...
    [delta, fxnew] = Directional_Newton(f, x, g, 1e-1*mu, n_quad_pts, false, fx, true);
    num_queries(k+1) = num_queries(k+1) + n_quad_pts;
%     [fxnew, delta] = Simple_LS(f, x, g, fx, n_LS);
%     num_queries(k+1) = num_queries(k+1) + n_LS;
    
    if isnan(fxnew) % should never happen..
        disp('errrrrrr(cars-dgs)');
        continue;
    end
