function [evals, Results] = more_test(ftns, rep, eps, noise_lvl, budget, verbose, showplot)
nftn = length(ftns);
evals = zeros(20, nftn);

for i=1:nftn % run for each function
    [tempevals, Results, finfo] = more_run(ftns(i), rep, eps, noise_lvl, budget, verbose, showplot);
